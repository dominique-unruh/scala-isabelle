...
class MLValue[A] protected (/** the ID of the referenced object in the Isabelle process */ val id: Future[Isabelle.ID])
   ...
   * */
    // TODO remove but keep docstring
  override def force : this.type = {
    Await.result(id, Duration.Inf)
    this
  }
  override def await: Unit = Await.result(id, Duration.Inf)
  override def someFuture(implicit ec: ExecutionContext): Future[Any] = id
   ...
   * @throws control.IsabelleException if the computation fails in the Isabelle process
   */
    // TODO: autogenerated by the FutureValue trait. Drop but keep docstring
  override def forceFuture(implicit ec: ExecutionContext) : Future[this.type] = id.map { _ => this }
